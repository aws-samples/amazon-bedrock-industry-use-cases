...
export class OpensearchStack extends cdk.Stack {
  public constructor(scope: cdk.App, id: string, props: any) {
    ...
    const kbRole = iam.Role.fromRoleArn(this, 'kb-role', cdk.Fn.importValue('kbRoleArn'));
    const customResourceRole = new iam.Role(this, 'index-custom-resource-role', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      managedPolicies: [iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSLambdaBasicExecutionRole')],
    });

    const accessPolicy = new opensearchserverless.CfnAccessPolicy(this, 'opensearch-access-policy', {
      ...
      type: 'data',
      description: 'Data access policy to grant access to Amazon Bedrock Knowledge Base.',
      name: 'smart-mfg-kb-access-policy',
    });

    const encryptionPolicy = new opensearchserverless.CfnSecurityPolicy(this, 'opensearch-encryption-policy', {
      ...
      name: 'smart-mfg-kb-encryption-policy',
    });

    const networkPolicy = new opensearchserverless.CfnSecurityPolicy(this, 'opensearch-network-policy', {
      policy: JSON.stringify([
        {
          Rules: [
      ...
      name: 'smart-mfg-kb-network-policy',
    });

    const opensearchCollection = new opensearchserverless.CfnCollection(this, 'opensearchCollection', {
      name: COLLECTION_NAME,
      description: 'Smart manufacturing Amazon Bedrock Knowledge Base vector database',
      type: 'VECTORSEARCH',
    });
    ...
    new cdk.CfnOutput(this, 'opensearch-arn', {
