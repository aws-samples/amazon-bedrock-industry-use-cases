...
def consolidate_results(
    ...
    for result in results:
        ...
        if completion and "Stage_Length" in completion[0]:
            normalized = normalize_operator1_result(result)
        elif completion and "Water" in completion[0]:
...
def query_wells(db_path: str = "well_data.db", query: str = None) -> pd.DataFrame:
    """
    Query the consolidated well database.

    Args:
        db_path: Path to SQLite database
        query: SQL query string. If None, returns all well information.
