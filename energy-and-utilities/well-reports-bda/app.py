...
with tab_consolidate:
    ...
    **The Value**: Different operators produce reports in different formats. 
    ...
    if not custom_outputs:
        ...
        custom_query = st.text_area("Or write your own SQL query (SELECT only):")
        if st.button("Run Query") and custom_query:
            is_valid, error_msg = validate_sql_query(custom_query)
            if not is_valid:
                st.error(f"Invalid query: {error_msg}")
            else:
                try:
                    # Query in-memory from the DataFrames using sqlite
                    import sqlite3
